...
- The root `requirements.txt` is autogenerated (`uv pip compile pyproject.toml -o requirements.txt`) — edit `pyproject.toml`, not it.
- `setup/` holds per-OS setup docs (`SETUP-mac.md`, `SETUP-PC.md`, `SETUP-linux.md`, `SETUP-WSL.md`) plus `diagnostics.py` and `troubleshooting.ipynb`; `guides/` holds course-topic notebooks (async Python, debugging, API/Ollama alternatives). Point users there for environment problems.
...
- `mcp_params.py` — declares MCP server stdio params. Trader stack: `accounts_server.py` + `push_server.py` + a market server (either local `market_server.py` or the Polygon MCP server run via `uvx` from GitHub, gated on `is_paid_polygon` / `is_realtime_polygon` in `market.py`). Researcher stack: `mcp-server-fetch` (uvx), Brave Search and libsql per-trader memory (both npx, memory in `./memory/<name>.db`) — so running the trading floor requires `uvx` and Node's `npx` on PATH.
